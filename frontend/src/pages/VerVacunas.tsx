...
import ModalEditarVacuna from "../components/modals/ModalEditarVacuna"
...
const VerVacunas: React.FC = () => {
  const [items, setItems] = useState<VacunaRow[]>([])
  ...
  const fetchVacunas = async (q?: string) => {
    try {
      setLoading(true)
      const resp = await vacunasAPI.getAll(q)
      const data = Array.isArray(resp.data) ? resp.data : Array.isArray(resp.data?.data) ? resp.data.data : []
      setItems(data as VacunaRow[])
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setTyping(false)
      setCurrentPage(1)
      fetchVacunas(query.trim() || undefined)
    }, 350)
  ...
  const handleDelete = async (id_vacuna: number, nombre: string) => {
    ...
    try {
      showLoadingAlert("Eliminando vacuna...", "Por favor espere")
      await vacunasAPI.delete(id_vacuna)
      setItems((prev) => prev.filter((v) => v.id_vacuna !== id_vacuna))
      closeLoadingAlert()
      await showSuccessAlert("¡Eliminada!", "La vacuna ha sido eliminada correctamente.")
    } catch (err: any) {
      closeLoadingAlert()
      const msg =
        err?.response?.status === 409
          ? "No se puede eliminar porque la vacuna ya fue aplicada en al menos una jaula."
          : err?.response?.data?.error || "No se pudo eliminar la vacuna. Inténtalo nuevamente."
  ...
  if (loading) {
    return <div className="ver-aves-container text-center">Cargando vacunas...</div>
  }
        ...
        <div className="header-content justify-between w-full">
          <div className="flex items-center gap-4">
            ...
            <div className="header-text">
              <h1 className="table-title">Listado de Vacunas</h1>
          ...
          <table className="tabla-aves">
            <thead>
              <tr>
                ...
                <th>
                  <span className="th-content">
                    <span className="th-icon">📅</span>Fecha administración
                  </span>
                </th>
                {muestraAcciones && (
            ...
            <tbody>
              {currentItems.map((v) => (
                <tr key={v.id_vacuna} className="table-row">
                  <td className="table-cell especie-cell">{v.nombre}</td>
                  <td className="table-cell text-center">{v.dosis}</td>
                  <td className="table-cell">{formatearFechaChilena(v.fecha_administracion)}</td>

                  {muestraAcciones && (
                    <td className="table-cell text-right">
                      <div className="acciones-cell justify-end">
                        {(userRole === "admin" || userRole === "operador") && (
                          <button className="btn-editar" onClick={() => openEdit(v.id_vacuna)}>
                            ✏️ Editar
                          </button>
                        )}
                        {userRole === "admin" && (
                          <button className="btn-eliminar" onClick={() => handleDelete(v.id_vacuna, v.nombre)}>
                            🗑️ Eliminar
                          </button>
                        )}
                      </div>
